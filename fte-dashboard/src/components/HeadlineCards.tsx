...
export default function HeadlineCards({ headline, context }: { headline: Headline; context: ReportContext }) {
  return (
    <div className="cards">
      <Stat label="Objectives (with FTE)" value={headline.objectivesWithAllocations} />
      ...
      {context.missingFte != null && <Stat label="Objectives missing FTE" value={context.missingFte} />}
    </div>
  );
}
